...
#show: template.with(
  ...
  short-title: "Written by Luuk Fröling",
  date: datetime(
    year: 2024,
    month: 1,
    day: 1,
...
#figure(
  image("files/sinogram2-4fe5b9d9d15b3f851681be6aafbc4af0.png", width: 90%),
  caption: [
_left:_ A phantom with multiple structures (ellipses) of different densities  _right_ corresponding sinogram. The darker regions in the sinogram indicate higher photon counts. _Figure reproduced from_ #cite(<leuschner2019lodopab>)
],
  kind: "figure",
  supplement: [Figure],
) <phantomsinogram>

A phantom and a corresponding sinogram can be seen in #link(<phantomsinogram>)[Figure~1]. The phantom contains multiple, elliptical structures of different densities. On the right is the corresponding sinogram where the darker parts of the sinogram correspond to more detected photons.

== Cone beam CT scan

The sinogram in #link(<phantomsinogram>)[Figure~1] is true for a 1D detector, where we construct a single slice. A cone beam CT scan uses a source emitting x-rays in a cone-beam shape, which can be detected on a 2D flat-panel detector #cite(<venkatesh2017cbct>).

#show figure: set block(breakable: breakableDefault)
#figure(
  image("files/cone_beam_ct-99b3680584828e6b48ebd5424f1fe103.png", width: 50%),
  caption: [
...
#link(<cone_beam>)[Figure~2] shows a cone beam CT scan setup, where both the detector and the source rotate around the object to acquire a set of 2D projection measurements at multiple angles. these measurements are known as projections and allow for a 3D image to be reconstructed.

The relationship between the object and the measured rays can be described by a projection matrix $A$. This matrix has dimensions $M times N$ where $N$ is the number of voxels in the object and $M$ is the number of rays (measurements) collected by the detector. Each element $a_(m n)$ of the matrix is the contribution of the $n^(t h)$ voxel of the object to be imaged to the $m^(t h)$ ray of the detector #cite(<Yang2017ProjectionMatrix>). The measured projections can thus be modeled as:

$ y = A x $ <projection_eq>
where $x$ is the vectorised representation of the voxel values and $y$ contains the corresponding ray measurements. This formulation will be used by the reconstruction algorithms during the simulations. To better reflect the statistical fluctuations during photon detection, noise is applied to the computed #link("http://projections.To")[projections.To] simulate measurement noise, each ideal projection value $y$ is used as the mean of a Poisson distribution from which a noisy measurement is sampled.

== Detector types

Currently, most CT scanners use an energy integrating detector (EID) #cite(<marth2023photon>). An EID measures the intensity of the incoming x-rays by a two-step process: scintillation followed by photodetection. The scintillator absorbs the x-rays which have passed through the body and emits light in the visible spectrum. These re-emitted photons can be detected by photodetectors located underneath the scintillator. Due to the difference in energy of the incoming x-ray and the emitted visible light, multiple light photons can be re-emitted. To measure a signal, EIDs integrate over time which loses all energy dependent information #cite(<taguchi2013photon>).

#show figure: set block(breakable: breakableDefault)
#figure(
  image("files/detectorTypes-1f23300987a3b96a7f3effc6cb05b528.png", width: 50%),
  caption: [
_left:_ An EID where the incoming x-rays are absorbed within the scintillator. Several photons within the visible light spectrum are re-emitted which can be measured by the photodiodes located below the scintillator. _right:_ A PCD where incoming x-rays create electron-hole pairs in a semiconductor. This creates a current between the top and bottom of the semiconductor which can be measured by electrodes, preserving energy dependent information.
],
  kind: "figure",
  supplement: [Figure],
) <detectors>

Photon-counting detector (PCD) CT scans improve the measurements by directly transforming the incoming X-ray photons into an energy-dependent signal, as illustrated in #link(<detectors>)[Figure~3]. In a PCD, each photon is absorbed in a semiconductor layer, generating electron-hole pairs that produce an electric signal proportional to the photon's energy. Unlike EIDs, PCDs can measure the energy of individual photons.

Commonly available X-ray sources emit a polychromatic spectrum #cite(<Antsiferov2003>), meaning the emitted photons span a broad range of energies rather than a single energy level. The PCD measures the energy of the incoming photons and sorts these into multiple energy bins, each representing a specific range within the spectrum #cite(<pcd2022>). By comparing the counts across these energy bins, multiple energy-resolved measurements can be done.

== Material decomposition

Since different materials attenuate x-rays differently across the energy spectrum, Spectral CT can be used to distinguish between different tissue types in the body. Spectral CT is a CT technique that leverages energy-dependent information to provide more comprehensive material differentiation in the produced images. To simulate this process and to describe the attenuation properties of different materials in a phantom, material decomposition is used #cite(<mechlem2018joint>). In clinical practice, human tissue can be approximated as a linear combination of two basis materials, commonly water and bone, which simplifies the decomposition process.

#show figure: set block(breakable: breakableDefault)
#figure(
  image("files/att_map-6b5577988a8a3212ab72b98e5006371b.png", width: 90%),
  caption: [
The linear attenuation coefficient ($mu$) of bone (green), water (blue) and iodine (orange) plotted as a function of the incoming X-ray photon energy. Iodine exhibits a k-edge, characterised by a sharp increase in attenuation at a specific energy, making it unsuitable to be modelled as a linear combination of bone and water. _Figure reproduced from_ #cite(<Willemink2018PhotonCountingCT>)
],
  kind: "figure",
  supplement: [Figure],
) <matatt>

The linear attenuation coefficient ($mu$), which describes how a material attenuates X-rays, depends on both the material and the photon energy, as shown by the attenuation curves in #link(<matatt>)[Figure~4]. For bone, water and other forms of tissue in the human body these attenuation curves can be assumed smooth within the energy range used #cite(<Willemink2018PhotonCountingCT>).  However, contrast agents such as iodine display one or more K-edges, sudden increases in attenuation at specific energies, which cannot be accurately modeled by a simple combination of water and bone. In this work, only bone and water will be used. If a contrast agent were to be involved, a 3rd material must be added to compensate for any possible k-edges.
...
$ mu (E) = sum_(b = 1)^B A_b f_b (E) $ <mue>
where $A_b$ is the weight of material b, representing the amount of material b present in the voxel, and $f_b (E)$ describes the attenuation coefficient of the basis material b. Combining equation #link(<first>)[(1)] and #link(<mue>)[(3)] gives an expression for the photon count at the detector:

$ hat(y)_i = integral_0^infinity phi.alt_("eff"comma i) (E) exp (-sum_(b = 1)^B A_b^i f_b (E)) thin d E $
where $phi.alt_("eff"comma i) (E)$ represents the effictive x-ray spectrum which includes both the shape of the X-ray source spectrum and how the detector responds to photons of different energies. The detector does not detect all photon energies equally well, some energies are absorbed more efficiently than others or may be missed due to noise or other effects. $A_b^i$ is the weight of material b along line path $L_i$.

== Projection algorithms

To view the cross-sectional image of an object from these measurements, either from a detector or from simulations using equation #link(<projection_eq>)[(2)], a reconstruction algorithm needs to be used. Filtered back projection (FBP) is an analytical reconstruction algorithm where during reconstruction each projection is spread back across the image plane along the same angle it was acquired. This often leads to blurry images as the intensity is spread along lines rather than concentrated at specific points #cite(<zeng2001image>). A filter is used to help sharpen the image and reduce blurring artifacts. FBP can be used for spectral CT scans by a two-step image based method. The first step is to reconstruct an image for each energy bin and these intermediate images are then decomposed into material-dependent images #cite(<mory2018comparison>).

The two-step, image based method has several drawbacks. First, streaking artifacts (lines across the image) often occur as the attenuation coefficient is still averaged over a line. Secondly, the first step leads to a loss of information as there is no one-to-one mapping between the projections and the images. The second step is unable to compensate for this loss as it has no access to the photon counts. Recently one-step methods have been proposed which reconstruct material-specific images directly from photon counts #cite(<mechlem2018joint>). These are all iterative methods as no analytical inversion formula exists.

#show figure: set block(breakable: breakableDefault)
#figure(
  image("files/itt-034b6f212be5f30f95f77cb82041b170.png", width: 90%),
  caption: [
An overview of an iterative reconstruction algorithm. From the measured projections a reconstructed image is estimated. This image is then iteratively compared with the original sinogram in the forward projection and corrected until a predefined endpoint. _Figure reproduced from_ #cite(<arndt2021deeplearning>)
],
  kind: "figure",
  supplement: [Figure],
) <itt>

An iterative reconstruction algorithm starts with an initial guess of the image and, at each iteration updates the image to better match the measured data. #link(<itt>)[Figure~5] shows the full cycle, from CT scan to final images. A forward projection as described by equation #link(<projection_eq>)[(2)] is used to simulate a sinogram for the guessed image. The simulated sinogram is compared with the measurements according to a cost function. The cost function depends on how well the image assumption explains the measured photon counts (data fidelity term), and how smooth, or physically plausible the images is (regularization term) #cite(<Zhang2018Regularization>). The iterative algorithm used, uses a separable quadratic surrogates (SQS) cost function. The guess is then updated and the cycle repeats untill a predefined endpoint.
...
#show figure: set block(breakable: breakableDefault)
#figure(
  image("files/neuralnetwork-7c664ffdc7b4a7b98a97346c5a4ef4fb.png", width: 90%),
  caption: [
...
All nodes in a layer are connected to an adjacent layer by weights. #link(<neural_network>)[Figure~6] shows a simple neural network, where input layer x and first hidden layer h' are connected by a matrix of weights $W_1$, the states of the first hidden layer can be calculated as

$ bold(h)_1 = sigma (bold(W)_1 bold(x) + bold(b)_1) $ <denselayer>
with $bold(W)_(1 comma n m)$ representing the learnable weights connecting the m-th input neuron to the n-th neuron in the first hidden layer, and $sigma$ is an activation function (e.g., sigmoid or softmax) that scales the output between 0 and 1 to stabilise the network and to introduce non-linearity. The bias term $bold(b)_1$ is another learned parameter which offsets the output. Equation #link(<denselayer>)[(5)] can be repeated for every following layer where the output of a layer is used as an input to calculate the following layer.
...
The kernel is moved across the input data, performing element-wise multiplication with the region of the input it covers, followed by a sum. This operation extract features such as edges, textures and shapes #cite(<yamashita2018convolutional>). We can write the output feature map of a convolutional layer as

$ S (i, j) = (bold(K) * bold(X)) (i, j) = sum_m sum_n bold(K) (m, n) dot.op bold(X) (i + m, j + n) $
where $(i, j)$ are spatial indices, $bold(X)$ the image, $bold(K)$ the relevant kernel with learnable weights and $(m, n)$ the kernel indices. After a convolution layer, $S (i, j)$ can be either vectorised to be used as an input for a fully connected layer as described in equation #link(<denselayer>)[(5)] or used as an input for a second convolution layer. Due to a kernel only working on adjecent pixels, convolutional layers extract local features.
...
label: <convex>,
  caption: [
_left :_ original phantom with a 2D 3x3 randomly initialised kernel drawn on top. each individual square represents a pixel, with the outer square representing the kernel. _right :_ phantom after 2D convolution layer has been applied.
],
  kind: "figure",
  supplement: [Figure],
)

The left part of #link(<convex>)[Figure~7] shows a 3x3 convolution kernel with randomly initialised weights (red) placed on a phantom and the right part of the figure shows the image of the phantom after the convolution. Even with randomly initialised weights, there is already an edge-detection like pattern. The image on the right is called a feature map, as each pixel represents higher-level information of the original image.
...
#show figure: set block(breakable: breakableDefault)
#figure(
  image("files/Unet-b453ddb3b21672f27228e873588d59c4.png", width: 75%),
  caption: [
A U-Net architecture for an image with a 32x32 lowest possible resolution. Each blue square represents a multi-channel feature map with the amount of channels denoted on top of the box. Gray arrows represent cross over layers with the white boxes representing copied layers from the encoder _Figure reproduced from_ #cite(<oktay2018attention>)
],
  kind: "figure",
  supplement: [Figure],
) <unet>

By combining an encoder and a decoder, a U-Net can be constructed as illustrated in #link(<unet>)[Figure~8]. Between each layer of the encoder and decoder part of the network, there is a skip-connection layer which copies the feature map from the encoder layer to the decoder layer. This allows the decoder to use features during reconstruction which might have been lost while encoding the image.

== Machine learning - Attention gate

For feature extraction from images where both local and non-local features are of importance, convolution layers can be combined with attention mechanisms. An attention mechanism allows machine learning models to attend to the most relevant parts of the input data #cite(<vaswani2017attention>).

An attention mechanism takes the input vector $x_i^l$ and multiplies it by an attention score $alpha_i^l$ to preserve only the activations relevant to the task. To calculate $alpha_i^l$,  an intermediate attention score $q_("att")^l$ (the attention score $q_("att")$ for layer l) can be defined as

$ q_("att")^l (x_i^l, g_i) = psi^T thin sigma (W_x^T x_i^l + W_g^T g_i + b_g) + b_psi $
where $g_i$ the gating feature vector, $W_x^T$ and $W_g^T$ weight matrices similar to those used in equation #link(<denselayer>)[(5)], $psi^T$ another learned linear transformation and $sigma$ an activation function. The gating feature vector provides contextual information from higher-level layers. It acts as a control signal that guides the attention mechanism, helping it decide which parts of the input to focus on. The final attention coefficient $alpha_i^l$, the final attention mask that says which parts of the feature map should be passed forward, is then calculated as

$ alpha_i^l = sigma (q_("att")^l (x_i^l, g_i \; Theta_("att"))) $
with $Theta_("att")$ representing the learned parameters of the attention block: all weights and biases. The final output of the attention gate is

$ tilde(x)_i^l = alpha_i^l dot.op x_i^l $
where regions get $alpha = 1$ where less important regions get $alpha approx 0$

#show figure: set block(breakable: breakableDefault)
#figure(
  image("files/dog-90a3cc53b4e9c683fe6e5d12d3707cd0.png", width: 90%),
  caption: [
Example image from an attention-based network trained to label images. Image a shows a picture of a dog which the network needs to label and image b shows the attention map which is the value of $alpha$ per pixel.  _Figure reproduced from_ #cite(<An2022Attention>)
],
  kind: "figure",
  supplement: [Figure],
) <dog>

An example has been taken from #cite(<An2022Attention>), where an attention-based network has been trained to label images. An input image can be seen on the left in #link(<dog>)[Figure~9] where the label is supposed to be 'dog'. A heatmap for the value of $alpha$ is shown on the right where we can see the attention mechanism provides a strong focus on the part of the image where the dog is located.

== Machine learning - Attention U-Net

Attention mechanisms can be added to the skip-connections of the U-Net model in #link(<unet>)[Figure~8], allowing the network to focus on non-local features during the reconstruction of the image. The combination of attention mechanisms and convolutional layers can be used by the attention U-Net (AttU-Net) to recognise certain features like edges and to make connection between features in different parts of the image #cite(<oktay2018attentionunet>).

#show figure: set block(breakable: breakableDefault)
#figure(
  image("files/AttU-Net-86c6f656da029e6df27f6848620cbcbe.png", width: 90%),
  caption: [
Schematic overview of a U-Net model with attention gates added to the skip-connections. The inset (top) shows a zoomed-in view of an attention gate, the gating signal $g$ is taken from the previous decoding layer and the input $x^l$ is the skip-connection layer. _Figure reproduced from_ #cite(<oktay2018attentionunet>)
],
  kind: "figure",
  supplement: [Figure],
) <attunet>

The attention mechanism uses the gating signal $g$ to modulate the features passed through the skip connection, which allows the network to selectively emphasize relevant spatial features and suppress irrelevant or noisy information. Each skip connection uses the output of the previous decoder layer as a gating signal to control which encoder features are passed forward, as illustrated in #link(<attunet>)[Figure~10].

= Method

== Overview

A machine learning model will be trained to use information from sinogram space to adjust images in image space. The network will be trained to perform this conversion in iterative steps similar to the iterative reconstruction method described in #cite(<mechlem2018joint>).

#show figure: set block(breakable: breakableDefault)
#figure(
  image("files/model2-ec3da612d514bb3090ae242dc099057b.png", width: 90%),
  caption: [
Schematic overview of the model workflow. The input to the model consists of the first reconstructed image from the iterative algorithm (called iteration 0 for the AttU-Net model) combined with the corresponding projection (sinogram) data. The network processes this input iteratively, where the output of the $n^(t h)$ iteration is used as the input for the $(n + 1)^(t h)$ iteration, while the projection data remains constant. After N iterations, the final reconstructed image is produced.
],
  kind: "figure",
  supplement: [Figure],
) <model2>

An overview of the model workflow is shown in #link(<model2>)[Figure~11]. The network is designed to use the first reconstructed image from the iterative algorithm (called iteration 0 for the AttU-Net model) combined with the projection matrix and refine the reconstructed image across 20 iterations. The output image of an iteration is used as an input for the following iteration, making sure the projection matrix remains constant. After 20 iterations, the output of the network is used as the final image.

== Generating phantoms

The training data consists of phantoms generated according to a set of rules for which projections are simulated. These projections are used by the iterative algorithm, where the intermediate images, as illustrated by the red, dotted line in #link(<itt>)[Figure~5], are used as training data for the model. The phantoms are randomly generated according to a set of rules:
...
Each feature represents a distinct elliptical region. Because photon counting detectors (PCDs) allow differentiating tissue types, we assign one feature a higher bone density and the other a higher water density. This design ensures that the network is trained on a task relevant to the clinical use of PCDs. These rules have been implemented in python to generate a sample of phantoms as seen in the supporting #link(<generate_phantom>)[notebooks].

== Acquiring data

For 506 phantoms, detector measurements are simulated. These measurements are acquired with 32 angles per phantom. Next, each set of detector measurements are reconstructed using the iterative algorithm, the predefined endpoint as shown in #link(<itt>)[Figure~5] is defined as 40 iterations. These iterations, illustrated by the red, dotted line in the same figure, are used to construct input-output pairs for training the model. Using a stride (how many iterations to skip between input and output images), the input is defined as the image from the $n^(t h)$ iteration, with a corresponding output image taken from the $(n + s t r i d e)^(t h)$ iteration.
...
To enable the network to learn how features in the image space and the projection (sinogram) space attend to eachother, a U-Net architecture with attention mechanisms integrated into the skip connections is used (AttU-Net) as described by #cite(<oktay2018attentionunet>). The attention gates help the model to focus on which parts of the sinogram contribute to specific image regions.

The AttU-Net model is illustrated in #link(<attunet>)[Figure~10]. The architecture consists of five convolutional encoder blocks, a bottleneck block, and five decoder blocks. The number of channels increases through the encoder (64, 128, 256, 512 and 1024), and then decreases symmetrically through the decoder. The output layer applies a sigmoid activation to produce voxel-wise probabilities. The model has been implemented in Python using Pytorch in the supporting #link(<network_code>)[notebooks]

The model is trained on an NVDIA Tesla A100, with 4 hours allocated per run. In total 6 runs were completed, which resulted in 180 epochs (20 epochs per run). The model uses MSELoss as a loss function and Adam optimisation, which is a stochastic gradient descent method. The learning rate is set to 0.001, which is commonly used with Adam optimisation #cite(<kingma2014adam>). An implementation of the training function can be seen in the supporting #link(<train_network>)[notebooks]

== Shaping the data

The AttU-Net model takes an image as input and produces an image of the same size as an output. Since 2 different images are used (the image and the projection), these must be combined into a single matrix. The projection matrix has dimensions (number of projections) x (z pixels detector) x (y pixels detector), where this work uses 32 projections and a detector of 64 by 44 pixels as set for the simulations in the supporting #link(<settings>)[notebooks].
...
$ "RMS_error" = sqrt(frac(1, N) sum_(i = 1)^N (I_1 (i) -I_2 (i))^2) $
...
#figure(
  ...
  caption: [
Top row (water), left to right: Ground truth (GT) water image\; iteration 0 (first reconstruction) from iterative algorithm\; subsequent reconstructions by the model. Bottom row (bone), left to right: GT bone image\; iteration 0 (first reconstruction) from iterative algorithm\; subsequent reconstructions by the model.
],
  kind: "figure",
  supplement: [Figure],
) <bone_recons>

The first 4 reconstructions of the second phantom are displayed in #link(<bone_recons>)[Figure~12] and the first column displays the ground truth (GT) images as a reference. The rows show the reconstruction process using the model for water and bone respectively, where each column is an iteration. In the GT images, distinct features are visible: a bone structure in the lower left quadrant and a water structure in the top right quadrant. These serve as reference to evaluate the reconstruction quality.
...
-
  ...
  caption: [
The average time (in seconds) required for each method to reach a specified error threshold $e$, calculated for 20 sets.
],
  kind: "table",
  supplement: [Table],
) <table>
...
The average values shown in #link(<table>)[Table~1] are highly influenced by the fluctuating range of error values produced by both methods. As seen in #link(<rmstime>)[Figure~14], the maximum error across methods for phantom 1 is 0.29 while this is 0.66 for phantom 3. To provide a more meaningful comparison between methods, we define an average speedup as the ratio between the time the iterative approach takes to reach the same RMS error that the AttU-Net model achieves after one iteration, and the time taken for that one AttU-Net iteration. Based on this definition:
...
Inpsecting all reconstructed images of the 20 phantoms as done in the supporting #link(<inspection>)[notebook], it can be noted some reconstructions performed by the iterative algorithm are visually far from the GT.
...
An attention map similar to #link(<dog>)[Figure~9] can be generated for the first skip connection of the AttU-Net model.
...
#link(<attention_network>)[Figure~16] shows an attention map from the trained AttU-Net model. The map visualizes the attention coefficients $alpha_i^l$ for the first skip connection. In this specific case, the attention gate focuses on the lower-intensity regions of the sinogram, which are known to contain the most informative features for reconstructing the imaged object.

= Discussion

== Model is only as good as the data

The trained AttU-Net model reconstructed 20 images using a validation set of phantoms. The first four reconstructions of the second phantom are shown in #link(<bone_recons>)[Figure~12] where the model successfully identifies features present in the phantom but fails to clearly distinguish between bone and water.

Looking at the RMS error over time for the first three phantoms as plotted in #link(<reconphantom>)[Figure~13], reveals that the AttU-Net model initially achieves a lower RMS error compared to the iterative algorithm but eventually converges to a higher value.

When analyzing the final iterations of the iterative algorithm across all 20 validation phantoms, it can be seem some images do not converge to a visually accurate state as displayed in figure #link(<wrong>)[Figure~15]. Since the AttU-Net model was trained on these results, including those that failed to converge, this likely explains why the model itself also fails to converge effectively in later iterations. For future work, it is recommended to set an RMS error threshold and use only iterative reconstructions that meet this criterion for training.
...
The model currently reconstructs images of size 32 $times$ 32 $times$ 32 pixels, which is too small for clinical applications. Future work should explore how performance and reconstruction time scale with increasing image size. Key factors to consider include:
...
Cross attention can implemented between two images as done by #cite(<alaluf2023cross>), where features from one image are used as a gating signal for an attention mechanism that processes the other image. Currently, two different spaces, image space and projection space, are added together in the same matrix. By using cross-attention, these spaces can be treated separately, with the attention mechanism serving as the interface between them. This allows for a more structured and interpretable flow of information between domains.

== Regularisation term

Since the AttU-Net is used to reconstruct CT scan images, its outputs must be clinically meaningful and physically plausible. To help guide the model toward producing more realistic reconstructions, a regularisation term can be introduced during training and reconstruction #cite(<ge2023mb>). This term can incorporate physical constraints into the learning process, encouraging the model to generate outputs that not only appear correct but also align with the physical reality of CT imaging. T
